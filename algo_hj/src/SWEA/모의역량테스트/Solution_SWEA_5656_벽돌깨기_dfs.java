...
public class Solution_SWEA_5656_벽돌깨기_dfs {
	...
	// 이부분 bfs로 처리했는데 여기를 dfs로 처리
	private static void boom(int[][] newMap, int r, int c, int cnt) { // cnt: r,c 의 숫자

		// dfs 로 처리하기
		newMap[r][c] = 0;
		if(cnt == 1) { // 주변 벽돌을 터트릴 수 없음 // 현 위치의 벽돌숫자가 1이면 주변 연쇄작용이 없으므로 그냥 리턴
			return;
		}
